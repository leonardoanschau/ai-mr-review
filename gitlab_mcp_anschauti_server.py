...
"""
GitLab MCP Server
MCP Server with GitLab tools.
Includes: GitLab Issue Creator, Project Lister, and more.
"""
...
def _make_gitlab_request(url: str, token: str, method: str = "GET", **kwargs) -> requests.Response:
    ...
    try:
        ...
        log_info(f"Response content-type: {response.headers.get('content-type', 'unknown')}")
        log_info(f"Response text preview (200 chars): {response.text[:200]}")
...
def get_projects_from_group(group_path: str, token: str, api_url: str) -> list[dict]:
    ...
    try:
        ...
        log_error(f"Erro ao fazer parse do JSON da resposta")
        log_error(f"Response status: {response.status_code}")
        log_error(f"Response text (primeiros 500 chars): {response.text[:500]}")
        raise ValueError(f"Resposta inválida do GitLab (não é JSON válido). Status: {response.status_code}")
